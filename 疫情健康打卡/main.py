...
def epidemic():
    ...
    try:
        ...
        if localtime()[3] != 7:  # 腾讯云函数的7点是+8时区的15点，15点是+8时区的7点
            return f'{name} 签到成功！'
...
def main(event, context):
    global name, username, password, agent, notify, token, msg
    now = datetime.now()
    ...
    for i in range(len(user_data)):
        ...
        msg += epidemic() + "\n"
    if localtime()[3] == 15:  # 腾讯云函数的7点是+8时区的15点，15点是+8时区的7点
        url = 'http://www.pushplus.plus/send'
...
if __name__ == '__main__':
    name, username, password, agent, notify, token, msg = "", "", "", "", "", "", ""
    main("", "")
